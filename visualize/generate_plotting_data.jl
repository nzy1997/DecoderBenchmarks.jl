...
function draw_fig6()
    ...
    ax = Axis(fig[1,1],xscale = log10,yscale = log10, xlabel = "Physical error rate", ylabel = "Logical error rate", title = "cg = 10, qubit_num = 1400, bb code x^-1*y")
    ax2 = Axis(fig[1,3],xscale = log10,yscale = log10, xlabel = "Physical error rate", ylabel = "decoding time per sample(ms)", title = "cg = 10, qubit_num = 1400, bb code x^-1*y")
    ...
    a = select_files_with_pattern(["code=bb","1000","TToricDecoder(Matching)","workers"])
    for filename in a
        @show filename
        data = JSON.parsefile(filename)
        pvec = data["pvec"][2:end]
        time_res = data["time_res"][2:end]
        scatterlines!(ax2, pvec, time_res, label="Toric, sample = $(data["nsample"])")
    end
    fig[1, 2] = Legend(fig, ax,)
    fig[1, 4] = Legend(fig, ax2,)
    fig
end
