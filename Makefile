...
benchmark-TensorQEC:
	$(JL) -e 'using DecoderBenchmarks;using TensorQEC; run_benchmark($(codevec), $(pvec), $(nsample), $(decoder), joinpath(@__DIR__,"data","result","TensorQEC"), joinpath(@__DIR__,"data","depolarizing");log_file="log.txt", filename_prefix=joinpath(@__DIR__,"data","result","files.txt"))'
...
benchmark-TensorQEC-Gurobi:
	$(JL) -e 'using DecoderBenchmarks,Gurobi;using TensorQEC; run_benchmark($(codevec), $(pvec), $(nsample), IPDecoder(Gurobi.Optimizer,false), joinpath(@__DIR__,"data","result","TensorQEC"), joinpath(@__DIR__,"data","depolarizing");log_file="log.txt", filename_prefix=joinpath(@__DIR__,"data","result","files.txt"))'
