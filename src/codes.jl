"""
    generate_code_data(code::QuantumCode,dirname::String)

Save the parity check matrix and logical operators for the code `code` and save them. 
    
The parity check matrix is saved to `joinpath(dirname, "pcm.txt")` in the form of `(X|Z)`. The size of the matrix is `(s, 2*n)`. `s` is the number of stabilizers, and `n` is the number of physical qubits. Each row of the matrix is a stabilizer. The first `n` columns represent whether this stabilizer contains a X operator on the physical qubit, and the last `n` columns represent whether this stabilizer contains a Z operator on the physical qubit.

The logical operators are saved to `joinpath(dirname, "logical_x.txt")` and `joinpath(dirname, "logical_z.txt")`. The size of the matrix is `(k, n)`. `k` is the number of logical operators, and `n` is the number of physical qubits. Each row of the matrix is a logical operator.
...
function generate_code_data(code::TensorQEC.QuantumCode,dirname::String)
    ...
    writedlm(joinpath(dirname, "logical_x.txt"),Int.(lx))
    writedlm(joinpath(dirname, "logical_z.txt"),Int.(lz))
end