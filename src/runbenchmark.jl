...
"""
function run_benchmark(code::TensorQEC.QuantumCode, pvec::AbstractVector, max_sim::Int, max_error::Int, decoder::TensorQEC.AbstractDecoder, result_dir::String; log_file = nothing)
	time_res = Float64[]
	error_rate = Float64[]
	nsim_res = Int[]
	error_count_res = Int[]
	...
	println("benchmark start code=$(code) decoder=$(decoder) workers=$(worker_count) max_sim=$(max_sim) max_error=$(max_error) pvec=$(pvec)")
	...
	for p in pvec
		...
		println("p=$(p) done nsim=$(nsim) error_count=$(error_count) avg_time=$(avg_time) err_rate=$(err_rate)")
	end
	data = Dict(
		"code_name" => "$code",
		"pvec" => pvec,
	...
	pmin = minimum(pvec)
	pmax = maximum(pvec)
	write(joinpath(result_dir, "code=$(code)_pmin=$(pmin)_pmax=$(pmax)_nsample=$(max_sim)_maxerror=$(max_error)_workers=$(worker_count)_decoder=$(decoder).json"), JSON.json(data))
...
function run_benchmark_time(code::TensorQEC.QuantumCode, pvec::AbstractVector, max_sim::Int, decoder::TensorQEC.AbstractDecoder, result_dir::String; init_num = 100)
	time_res = Float64[]
	mkpath(result_dir)
	...
	for p in pvec
		println("p=$(p) start")
		time_sum = 0.0
        em = iid_error(p / 3, p / 3, p / 3, tanner.stgz.nq)
        for i in 1:(init_num + max_sim)
            eq = random_error_pattern(em)
			...
			i > init_num && (time_sum += time_end - time_start)
        end

		avg_time = time_sum / max_sim
		push!(time_res, avg_time)
	end
	data = Dict(
		"code_name" => "$code",
	...
	pmin = minimum(pvec)
	pmax = maximum(pvec)
	write(joinpath(result_dir, "Time_code=$(code)_pmin=$(pmin)_pmax=$(pmax)_nsample=$(max_sim)_decoder=$(decoder).json"), JSON.json(data))
	return nothing
end
